          ...
          {activeSection === "platforms" && (
            <div>
              ...
              {platforms.filter(p => !filterText || JSON.stringify(p).toLowerCase().includes(filterText)).map((p, i) => (
                <Accordion key={i} title={<span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><PlatformLogo platform={p.name.split(" ")[0] === "Google" ? "Google" : p.name.split(" ")[0] === "YouTube" ? "YouTube" : p.name.split(" (")[0]} />{p.name}</span>} badge={<Chip color={p.color}>{p.minBudget} min</Chip>}>
                  <div style={{ fontSize: 13, color: "#555566", lineHeight: 1.7 }}>
