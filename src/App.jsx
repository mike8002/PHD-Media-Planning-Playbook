...
const PhoneMockup = ({ ratio, label, dims, color: bgColor = "#e8e0f0", children: overlay }) => (
  <div style={{ width: 95, display: "inline-flex", flexDirection: "column", alignItems: "center" }}>
    <div style={{ width: 82, background: "#f0f0f4", borderRadius: 12, border: "1.5px solid #d0d0d8", padding: "5px 4px" }}>
    ...
    <div style={{ fontSize: 9, color: "#444455", marginTop: 5, fontWeight: 600, textAlign: "center", lineHeight: 1.2 }}>{label}</div>
  </div>
);
const BannerMockup = ({ w, h, label }) => (
  <div style={{ display: "inline-flex", flexDirection: "column", alignItems: "center" }}>
    ...
    <div style={{ fontSize: 8, color: "#444455", marginTop: 3, fontWeight: 600 }}>{label}</div>
  </div>
);
const ScreenMockup = ({ type, label, children: overlay }) => (
  <div style={{ width: 110, display: "inline-flex", flexDirection: "column", alignItems: "center" }}>
    ...
    <div style={{ fontSize: 9, color: "#444455", marginTop: 4, fontWeight: 600, textAlign: "center" }}>{label}</div>
  </div>
);
...
const programmaticEcosystem = [
  ...
  { category: "CTV / Connected TV", description: "Connected TV and streaming platforms offering ad-supported inventory by region.", vendors: [
    ...
    { name: "Skyscale", type: "CTV / Video Network", what: "Multi-format video and CTV platform. Premium inventory across multiple markets and streaming environments.", gcc: "Available WEST and EAST. Cross-regional CTV capabilities.", bestFor: "Cross-regional CTV, multi-market video", minBudget: "Contact vendor", regions: ["WEST","EAST"] },
  ...
  { category: "AUDIO", description: "Audio and music streaming advertising platforms.", vendors: [
    ...
    { name: "NBM (National Business Media)", type: "Audio Network", what: "Audio advertising network covering premium radio and streaming inventory.", gcc: "Primarily WEST region.", bestFor: "Radio and streaming audio, brand awareness", minBudget: "Contact vendor", regions: ["WEST","CENTRAL"] },
  ...
  { category: "DOOH (Digital Out-of-Home)", description: "Programmatic and direct digital billboard and screen networks by region.", vendors: [
    ...
    { name: "NEURONS", type: "Attention / Creative Analytics", what: "AI-powered attention prediction platform. Predicts how consumers will visually engage with creative before launch. Optimises DOOH and digital creative for attention and recall.", gcc: "Available globally. Use to pre-test DOOH and digital creative for attention metrics.", bestFor: "Attention prediction, creative optimisation, DOOH pre-testing", minBudget: "License-based", regions: ["WEST","CENTRAL","EAST"] },
  ]},
...
const regionalVendors = {
  ...
  "EAST (APAC / China / India)": { desc: "Vendors with availability in Asia-Pacific, China, and Indian markets.", data: programmaticEcosystem.map(cat => ({...cat, vendors: cat.vendors.filter(v => v.regions?.includes("EAST"))})).filter(cat => cat.vendors.length > 0) },
};
          ...
          {activeSection === "platforms" && (
            <div>
              ...
              {platforms.filter(p => !filterText || JSON.stringify(p).toLowerCase().includes(filterText)).map((p, i) => (
                <Accordion key={i} title={`${p.icon || ""} ${p.name}`} badge={<Chip color={p.color}>{p.minBudget} min</Chip>}>
                  <div style={{ fontSize: 13, color: "#555566", lineHeight: 1.7 }}>
                    ...
                    {platformMockups[p.name] && (
                      <div style={{ marginTop: 16, padding: "16px 0 4px", borderTop: "1px solid #e0e0e8" }}>
                        ...
                        <div style={{ transform: "scale(1.6)", transformOrigin: "top left", marginBottom: 60 }}>
                          {platformMockups[p.name]()}
                        </div>
                      </div>
                    )}
                    ...
                    <div style={{ width: "100%", background: "linear-gradient(135deg, #f0e8d8, #e8e0f0)", borderRadius: 3, border: "3px solid #504020", overflow: "hidden" }}>
                      <div style={{ paddingBottom: "50%", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
                        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column" }}>
                          ...
                          <div style={{ fontSize: 8, color: "#444455", marginTop: 3, fontWeight: 600 }}>10–15s loops</div>
